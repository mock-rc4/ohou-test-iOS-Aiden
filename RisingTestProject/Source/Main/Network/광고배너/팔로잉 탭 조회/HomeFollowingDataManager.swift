//
//  HomeFollowingDataManager.swift
//  RisingTestProject
//
...
final class HomeFollowingDataManager {
    
    func getFollowingPost(delegate: HomeFollowingVC) {
        
        let url = "\(Constant.baseURL)/board"
        ...
        AF.request(url,
                   ...
                   parameters: nil,
                   headers: header)
        .responseDecodable(of: HomeFollowingResponse.self) { response in
            switch response.result {
            case .success(let response):
                if response.isSuccess {
                    print("데이터 가져오기 성공")
                    delegate.followingPost = response.result
